...
const onSubmit = async () => {
  ...
  loading.value = true
  try {
    // 1) Login -> devuelve token y usuario
    const { data } = await api.post('/login', {
      email: email.value,
      password: password.value,
    })

    // 2) Guardar token y usuario en storage
    localStorage.setItem('token', data.token)
    localStorage.setItem('user', JSON.stringify(data.usuario))

    // 3) Validar token una vez contra el backend y marcar cache de pestaña
    try {
      await api.get('/me')
      sessionStorage.setItem('tokenChecked', '1')
    } catch {
      // Si el token no es válido por alguna razón, limpiamos y mostramos error
